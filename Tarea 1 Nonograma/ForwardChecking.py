...
def forward_checking(index, domains, width, height, row_clues, col_clues, node_counter):
    ...
    for value in domains[index]:
        ...
        if is_consistent(assignment, index, value, row_clues, col_clues, width, height):
            ...
            if all(new_domains[i] for i in range(index + 1, len(domains))):
                result, new_node_counter = forward_checking(index + 1, new_domains, width, height, row_clues, col_clues, node_counter)
                if result:
...
def nonogram_solver(row_clues, col_clues):
    ...
    new_domains, node_counter = forward_checking(0, domains, width, height, row_clues, col_clues, 0)
